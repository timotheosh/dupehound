...
/// — any language whose `classify()` returns `Op` reuses this unchanged.
const OP_BASE: u16 = u16::MAX - 4099;
const OP_BAND: u16 = 4096;

/// Code for an `Op`-classified leaf: same text always hashes to the same
/// code, so `+` still means `+` everywhere it's the head of a Lisp form.
...
pub fn significant_lines(node: Node, src: &[u8]) -> u32 {
    let mut count = 0u32;
    let mut last_line = u32::MAX;
    visit_leaves(node, &mut |leaf| {
        if classify(leaf, src) == TokenClass::Comment {
            return;
        }
        let row = leaf.start_position().row as u32;
        if row != last_line {
...
mod tests {
    ...
    fn op_code_stays_inside_the_reserved_band_and_off_the_sentinels() {
        for text in [
            "+", "-", "*", "/", "if", "when", "let", "reduce", "filter", "map", "defun",
        ] {
